...
Версия: WB_WAREHOUSE_LOSS_REGISTRY_V2_20260805
...
6. Для проблемных складов определить дату устойчивой остановки заказов либо использовать
...
Обычный запуск за 2026 год с отправкой файла в Telegram:
    ...
    python wb_warehouse_loss_registry_TOPFACE_V2_20260805.py
...
Не отправлять в Telegram:
    ...
    python wb_warehouse_loss_registry_TOPFACE_V2_20260805.py --no-telegram
"""
...
VERSION = "WB_WAREHOUSE_LOSS_REGISTRY_V2_20260805"
...
DEFAULT_INCIDENT_WAREHOUSES = "Владимир,Рязань,Котовск,Электросталь"
IN_WAY_TO_NAMES = {
...
def resolve_incidents(
    ...
    # Автоопределение нельзя вести до date_to, если недельные файлы заказов
    # ещё не содержат последние дни. Используем последнюю реально загруженную
    # дату заказов по всем складам как границу наблюдения.
    ...
    for warehouse_raw in incident_warehouses:
        warehouse = canonical_warehouse(warehouse_raw)
        if not warehouse:
            continue
        warehouse_orders = orders[
            (orders["warehouse"] == warehouse) & (~orders["is_cancel"])
        ].copy() if not orders.empty else pd.DataFrame()

        order_dates = sorted(set(warehouse_orders["order_date"].tolist())) if not warehouse_orders.empty else []
        ...
        if manual_value not in {None, "", "auto", "AUTO"}:
            ...
            if not incident_date:
                raise ValueError(
                    f"Некорректная дата инцидента для {warehouse}: {manual_value}"
                )
            orders_after = 0
            if not warehouse_orders.empty:
                orders_after = int((warehouse_orders["order_date"] >= incident_date).sum())
            warning = ""
            confidence = "Высокая"
            if orders_after > 0:
        ...
        if not stocks_history.empty:
            has_stock_history = bool(
                (
                    ...
                    & (stocks_history["quantity"] > 0)
                ).any()
            )
        can_detect = (
            last_order_date is not None
            and quiet_days >= shutdown_confirm_days
            and active_order_days >= min_active_order_days
            and active_orders >= min_active_orders
            and has_stock_history
        )
        if can_detect:
            incident_date = last_order_date + dt.timedelta(days=1)
            confidence = "Средняя"
            source = (
                f"Авто: последний неотменённый заказ {last_order_date.isoformat()}, "
                f"далее {quiet_days} дней без заказов"
            )
            warning = "Дата определяется по остановке заказов, а не по официальному акту"
        else:
            incident_date = None
            confidence = "Недостаточно данных"
            source = "Автоопределение не подтверждено"
            reasons = []
            if last_order_date is None:
                reasons.append("нет заказов")
            if quiet_days < shutdown_confirm_days:
                reasons.append(f"тишина только {quiet_days} дн.")
            if active_order_days < min_active_order_days:
                reasons.append(f"активных дней {active_order_days}")
            if active_orders < min_active_orders:
                reasons.append(f"заказов {active_orders}")
            if not has_stock_history:
                reasons.append("нет положительного снимка остатков")
...
def run(args: argparse.Namespace) -> int:
    ...
    log(f"Проблемные склады: {', '.join(incident_warehouses) or 'не заданы'}")
