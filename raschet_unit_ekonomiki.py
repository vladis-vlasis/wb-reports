...
SCRIPT_VERSION = "2026-06-10_v15_TOPFACE_MT_UNIT_ECONOMICS"

def _normalize_store_name(value: str) -> str:
    raw = str(value or "").strip().upper().replace(" ", "").replace("-", "")
    if raw in {"MT", "МТ", "MISSTAIS", "MISSTAIS"}:
        return "MISSTAIS"
    if raw in {"TOPFACE", "TF", "ТФ"}:
        return "TOPFACE"
    return raw or "TOPFACE"


def _resolve_store_name() -> str:
    store = os.getenv("WB_STORE") or os.getenv("STORE") or os.getenv("MARKETPLACE_STORE") or "TOPFACE"

    argv = list(sys.argv or [])
    for idx, arg in enumerate(argv):
        if arg == "--store" and idx + 1 < len(argv):
            store = argv[idx + 1]
            break
    ...
    return _normalize_store_name(store)


МАГАЗИН = _resolve_store_name()
...
def загрузить_report_env() -> None:
    """Загружает KEY=VALUE строки из GitHub Secret REPORT_ENV в os.environ.

    Нужно для старой рабочей схемы, где YC_ACCESS_KEY_ID, YC_SECRET_ACCESS_KEY,
    YC_BUCKET_NAME и YC_ENDPOINT_URL лежат внутри одного REPORT_ENV.
    """
    raw = os.environ.get("REPORT_ENV", "") or ""
    if not raw.strip():
        лог("report_env: REPORT_ENV пустой/не передан; проверяю отдельные env/secrets")
        return
    ...
    интересные = ["YC_ACCESS_KEY_ID", "YC_SECRET_ACCESS_KEY", "YC_BUCKET_NAME", "YC_ENDPOINT_URL"]
    state = ", ".join(f"{k}={'set' if (os.environ.get(k) or '').strip() else 'empty'}" for k in интересные)
    лог(f"report_env: present=yes, loaded_keys={loaded}, skipped_existing={skipped}, bad_lines={bad_lines}")
    лог(f"env_state: {state}")
...
def main():
    лог(f"VERSION: {SCRIPT_VERSION}")
    лог(f"STORE: {МАГАЗИН}")

    загрузить_report_env()

    required_env = ["YC_ACCESS_KEY_ID", "YC_SECRET_ACCESS_KEY", "YC_BUCKET_NAME"]
    missing = [var for var in required_env if not os.environ.get(var)]
    if missing:
        raise RuntimeError(f"Отсутствуют переменные окружения: {missing}. Проверь REPORT_ENV или отдельные GitHub secrets.")
