...
Версия: WB_WAREHOUSE_LOSS_REGISTRY_V4_TELEGRAM_FIX_20260805_1330
...
Обычный запуск за 2026 год с отправкой файла в Telegram:
    ...
    python wb_warehouse_loss_registry_TOPFACE_V4_TELEGRAM_FIX_20260805_1330.py
...
Не отправлять в Telegram:
    ...
    python wb_warehouse_loss_registry_TOPFACE_V4_TELEGRAM_FIX_20260805_1330.py --no-telegram
"""
...
VERSION = "WB_WAREHOUSE_LOSS_REGISTRY_V4_TELEGRAM_FIX_20260805_1330"
...
def clean_secret_value(value: Any) -> str:
    """Удаляет кавычки, BOM и управляющие символы из GitHub Secrets/REPORT_ENV."""
    text = str(value or "").strip().strip('"').strip("'").lstrip("\ufeff")
    return re.sub(r"[\x00-\x1f\x7f]+", "", text).strip()


def telegram_credentials() -> Tuple[str, str, str]:
    token = clean_secret_value(first_nonempty(
        os.getenv("TELEGRAM_BOT_TOKEN"),
        os.getenv("TG_BOT_TOKEN"),
        os.getenv("TELEGRAM_TOKEN"),
    ))
    chat_id = clean_secret_value(first_nonempty(
        os.getenv("TELEGRAM_CHAT_ID"),
        os.getenv("TG_CHAT_ID"),
        os.getenv("CHAT_ID"),
    ))
    thread_id = clean_secret_value(first_nonempty(
...
def send_telegram_document(filename: str, data: bytes, caption: str) -> bool:
    ...
    try:
        response = requests.post(url, data=fields, files=files, timeout=180)
        response_text = response.text[:500]
        ok = 200 <= response.status_code < 300
        log(
            f"Telegram sendDocument: status={response.status_code}, "
            f"ok={ok}, response={response_text}"
        )
        return ok
...
def send_current_registry_only(args: argparse.Namespace, store: str, storage: S3Storage) -> int:
    current_key = f"Отчёты/Контроль потерь/{store}/Текущий_реестр.xlsx"
    try:
        xlsx = storage.read_bytes(current_key)
    except Exception as exc:
        raise RuntimeError(f"Не удалось прочитать готовый реестр {current_key}: {exc}") from exc

    filename = f"Текущий_реестр_WB_{store}_{today_msk().isoformat()}.xlsx"
    caption = (
        f"Повторная отправка готового реестра WB {store}.\n"
        f"Источник: {current_key}"
    )
    if args.no_telegram:
    ...
    if not send_telegram_document(filename, xlsx, caption):
        return 2
    log(f"Готовый реестр повторно отправлен в Telegram: {current_key}")
    return 0
...
def run(args: argparse.Namespace) -> int:
    ...
    if args.send_current_only:
        log(f"VERSION: {VERSION}")
        log(f"Режим: повторная отправка готового файла; магазин={store}")
        return send_current_registry_only(args, store, storage)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--send-current-only",
        action="store_true",
        help="Не пересчитывать отчёт, а отправить в Telegram уже сохранённый Текущий_реестр.xlsx",
    )
